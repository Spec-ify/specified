...
function createLinks(selector) {
	...
	document.querySelectorAll(selector).forEach((e) => {
		...
		const kebab = e.innerText
			.toLowerCase()
			.split(' ')
			.join('-')
			.replace(/[^a-zA-Z0-9-_]/g, '');
		let num = 1;
		while (Object.hasOwn(headings, `${kebab}-${num}`)) num++;
...
(async () => {
	...
	const cpuResponse = await call_hwapi(
		`api/cpus/?name=${encodeURIComponent(cpuName)}`,
		null,
		() => {
	...
	const [usbResponse, pcieResponse] = await Promise.all([
		usbResponsePromise,
		pcieResponsePromise
	]);
	...
	deviceTrs.forEach((tr, trIndex) => {
		...
		if (usbArrayIndex !== -1 && usbResponse[usbArrayIndex]) {
			// the value is a usb device and it was found
		...
		response.forEach((bugcheck, index) => {
			...
			if (bugcheck && bugcheck.name) {
				...
				const parent = document
					.querySelectorAll('#unexpected-shutdowns-table tbody tr')
	...
	document.querySelectorAll('#pcie-whea-table tbody tr').forEach((tr, trIndex) => {
		...
		for (const [index, device] of json['Hardware']['Devices'].entries()) {
			if (
				device['DeviceID'].includes(vendorString) &&
				device['DeviceID'].includes(deviceString)
			) {
				...
				jumpLink.onclick = () => {
					...
					document
						.querySelector(`#device-${index}`)
						.parentElement.classList.add('tr-focus');
					setTimeout(() => {
						document
							.querySelector(`#device-${index}`)
							.parentElement.classList.remove('tr-focus');
					}, 3000);
				};
